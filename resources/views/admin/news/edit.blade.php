...
<form action="{{ URL('/admin/news/'.$news->id) }}" class="form-horizontal" id="editor-form" method="POST">
	...
	<script>
		$(function(){
			...
			var input_title = '<ul class="list-group"><li class="list-group-item"><div class="form-group no-margin"><label for="" class="col-sm-2 control-label">新闻标题</label><div class="col-sm-4"><input type="text" class="form-control" name="title" value="{{ $news->title }}"/></div><label for="" class="col-sm-2 control-label">新闻分类</label><div class="col-sm-4"><select name="tag" id="" class="form-control"><option value="新闻动态" <?php if($news->tag == "新闻动态") echo "selected"; ?>>新闻动态</option><option value="通知公告" <?php if($news->tag == "通知公告") echo "selected"; ?>>通知公告</option><option value="人才招聘" <?php if($news->tag == "人才招聘") echo "selected"; ?>>人才招聘</option><option value="举办会议" <?php if($news->tag == "举办会议") echo "selected"; ?>>举办会议</option><option value="学术报告" <?php if($news->tag == "学术报告") echo "selected"; ?>>学术报告</option><option value="外出交流" <?php if($news->tag == "外出交流") echo "selected"; ?>>外出交流</option></select></div></div></li></ul>';
